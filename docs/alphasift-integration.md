...
- 依赖来源：固定到已验证的 AlphaSift 适配层 commit `1a0ed8c99b3615c0cb1076e6029827ffc6de2344`。安装方式因环境而异：
  - `docker/Dockerfile`：在 `apt-get install git` 之后单独执行 `pip install "alphasift @ git+https://..."`。
  - 桌面端构建脚本（`scripts/build-backend-macos.sh` / `scripts/build-backend.ps1`）：在 `pip install -r requirements.txt` 之后单独安装。
  - 本地开发：手动执行 `pip install "alphasift @ git+https://github.com/ZhuLinsen/alphasift.git@1a0ed8c99b3615c0cb1076e6029827ffc6de2344"`。
  - `requirements.txt` 中仅保留注释行作为安装来源参考，不再作为 `pip install -r` 的自动安装项（避免 Zeabur 等无 git 环境构建失败）。
