...
- 本文件为 Phase 0 合同与验收边界文档，不包含运行时代码变更与回归测试执行。
- 针对 `#1391` 的运行时修复，当前范围仅用于“问题边界固化”；若无 Token/代码映射约束要求，不在本轮扩展 Tushare `600/601/603/688` 之外的裸码归属规则（如 `605xxx`），该问题请在后续专门 PR 跟进修复。
