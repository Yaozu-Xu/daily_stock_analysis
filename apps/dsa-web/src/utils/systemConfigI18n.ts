...
const fieldTitleMap: Record<string, string> = {
  ...
  AGENT_EVENT_MONITOR_ENABLED: '告警中心（后台轮询）',
  AGENT_EVENT_MONITOR_INTERVAL_MINUTES: '告警轮询间隔（分钟）',
  AGENT_EVENT_ALERT_RULES_JSON: '事件告警规则（Legacy）',
...
const fieldDescriptionMap: Record<string, string> = {
  ...
  AGENT_EVENT_MONITOR_ENABLED: '是否启用 schedule 模式下的告警后台轮询。',
  AGENT_EVENT_MONITOR_INTERVAL_MINUTES: '轮询周期（分钟），关闭 Event Monitor 后不生效。',
  AGENT_EVENT_ALERT_RULES_JSON: 'legacy 告警规则 JSON，仅用于兼容运行时；推荐使用告警中心（Web/API）管理规则。',
